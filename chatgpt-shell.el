...
(defcustom chatgpt-shell-buffer-selection-function nil
  "Function to determine which chatgpt-shell buffer to use.

If nil (the default), `chatgpt-shell' uses the standard behavior:
- Commands like `chatgpt-shell-send-region' target the primary shell buffer
  (creating it if necessary).
- `M-x chatgpt-shell' switches to the primary shell buffer.
- `C-u M-x chatgpt-shell' creates a new, independent shell buffer.

If non-nil, this should be a function that takes no arguments and
returns a live buffer object. This function is called by commands
like `chatgpt-shell' (without C-u) or `chatgpt-shell-send-region'
to decide the target buffer.

The function is responsible for the selection logic (e.g., based
on `default-directory` or `projectile-project-root`). It should
find an existing suitable buffer or create and return a *new*,
properly initialized `chatgpt-shell` buffer if necessary.

To create a new buffer correctly within your function, call
`chatgpt-shell-start' with appropriate arguments (e.g.,
:new-session t, :ignore-as-primary t, and potentially a
:buffer-name-override).

Using the prefix argument (e.g., `C-u M-x chatgpt-shell`) will
*always* create a new buffer and bypass this function."
...
(defun chatgpt-shell--get-target-buffer (&optional force-new-p)
  "Determine the target chatgpt-shell buffer.

If FORCE-NEW-P is non-nil (e.g., from C-u prefix), always
create and return a new buffer, ignoring
`chatgpt-shell-buffer-selection-function'.

If `chatgpt-shell-buffer-selection-function' is set, call it
and return its result (must be a live buffer).

Otherwise, return the primary shell buffer (creating if needed)."
  (cond
   (force-new-p
    ;; Create a new session, ignoring primary status.
    ;; Potentially determine context for naming here.
    (let* ((context-name nil) ; Placeholder for project name logic
           ;; Example: Get project name if projectile is loaded
           (_ (when (fboundp 'projectile-project-p)
                (when (projectile-project-p)
                  (setq context-name (projectile-project-name))))))
      (chatgpt-shell-start :no-focus nil ; Focus the new buffer
                           :new-session t
                           :ignore-as-primary t
                           :buffer-name-override (chatgpt-shell--make-buffer-name context-name))))
   ((functionp chatgpt-shell-buffer-selection-function)
    ;; Call the user's function
    (let ((selected-buffer (funcall chatgpt-shell-buffer-selection-function)))
      (unless (buffer-live-p selected-buffer)
        (error "chatgpt-shell-buffer-selection-function did not return a live buffer"))
      ;; Ensure the buffer is actually a chatgpt-shell buffer? Optional check.
      (unless (with-current-buffer selected-buffer (derived-mode-p 'chatgpt-shell-mode))
         (warn "Buffer '%s' returned by selection function is not in chatgpt-shell-mode"
               (buffer-name selected-buffer)))
      selected-buffer))
   (t
    ;; Default behavior: get or create the primary buffer
...
With NEW-SESSION (prefix argument C-u), start a new session,
bypassing `chatgpt-shell-buffer-selection-function'.

Otherwise, use `chatgpt-shell-buffer-selection-function' if set
to determine the target buffer, or default to the primary shell."
  (interactive "P")
  ;; Get the target buffer using the new logic
  (let ((target-buffer (chatgpt-shell--get-target-buffer new-session)))
...
(defun chatgpt-shell-start (&optional no-focus new-session ignore-as-primary model-version system-prompt buffer-name-override)
...
Set BUFFER-NAME-OVERRIDE to force a specific buffer name."
  (let* ((chatgpt-shell--config
          (let ((config (copy-sequence chatgpt-shell--config))
            ...
            config))
         (target-buffer-name ;; Determine the buffer name
          (cond
           (buffer-name-override buffer-name-override) ;; Use override if provided
           ((and (not new-session) ;; Not forcing new, and have a primary?
                 (not ignore-as-primary) ;; And not ignoring primary
                 (chatgpt-shell--primary-buffer))
            (buffer-name (chatgpt-shell--primary-buffer))) ;; Use primary name
           (t ;; Otherwise, generate a name (potentially context-aware later)
            (chatgpt-shell--make-buffer-name)))) ;; TODO: Pass context here if needed
         (shell-buffer
          (shell-maker-start chatgpt-shell--config
                             no-focus
                             chatgpt-shell-welcome-function
                             new-session
                             target-buffer-name ;; Use the determined name
                             "LLM")))
    ;; Only set as primary if it's the *first* shell and not explicitly ignored
    ...
    (with-current-buffer shell-buffer
      ...
      (chatgpt-shell--update-prompt nil) ; Do not rename buffer if override was used
...
(defun chatgpt-shell--make-buffer-name (&optional context-name)
  "Generate a buffer name using current shell config info.
Optionally include CONTEXT-NAME in the buffer name."
  (format "*%s llm (%s%s)*"
          (downcase (chatgpt-shell--model-label))
          (if context-name (format "%s: " context-name) "") ;; Add context here
...
(defun chatgpt-shell-send-to-buffer (input &optional review handler on-finished response-style)
  "Send INPUT to a chatgpt-shell buffer determined by context or settings.

Set REVIEW to make changes before submitting to ChatGPT.

If HANDLER function is set, it receives the output directly.

RESPONSE-STYLE defaults to `chatgpt-shell-prompt-query-response-style'.
- 'inline: Inserts response into the current buffer.
- 'other-buffer: Uses `chatgpt-shell-prompt-compose'.
- 'shell: Sends to the appropriate shell buffer and focuses it.

The target shell buffer is determined by `chatgpt-shell--get-target-buffer`
unless RESPONSE-STYLE dictates otherwise (like 'inline' or 'other-buffer').
...
ON-FINISHED is invoked when the entire interaction is finished:
  ...
  (cond
   ;; Compose handles its own buffer logic
   ((eq response-style 'other-buffer)
    (let ((buffer (chatgpt-shell-prompt-compose-show-buffer :content input)))
      (unless review
        (with-current-buffer buffer
          (chatgpt-shell-prompt-compose-send-buffer)))))

   ;; Inline or Handler don't need a specific shell buffer *selected* initially
   ((or handler (eq response-style 'inline))
    (let* ((buffer (when (eq response-style 'inline) (current-buffer))) ; Target for inline insertion
           (marker (copy-marker (point)))
           (orig-region-active (region-active-p))
           (region-beginning (when orig-region-active (region-beginning)))
           (region-end (when orig-region-active (region-end)))
           ;; Get the shell buffer to *send* the command to (might be background)
           (shell-buffer (chatgpt-shell--get-target-buffer nil))) ; Use selection logic, no force new
      ) ; <<< This parenthesis closes the list of bindings
      (unless (buffer-live-p shell-buffer)
        (error "Could not get a valid chatgpt-shell buffer to send command"))
      (when (region-active-p)
        (setq marker (copy-marker (max (region-beginning) (region-end)))))

      ;; Ensure the target shell is running
      (with-current-buffer shell-buffer
         (unless (comint-check-proc shell-buffer)
            (chatgpt-shell-start :no-focus t))) ; Start if needed, no focus change

      ;; Send the command from the context of the shell buffer
      (with-current-buffer shell-buffer
         (cl-flet ((send ()
                     ...
                     (if review
                       ...
                       (shell-maker-submit
                        :input input
                        :on-output (lambda (output) ; Callback for inline/handler
                                     (setq output (or output ""))
                                     (when handler (funcall handler output)) ; Call handler if provided
                                     (when (and buffer (buffer-live-p buffer)) ; Check if inline target exists
                                       (with-current-buffer buffer
                                         (let ((inhibit-read-only t))
                                           (save-excursion
                                             ...
                                             (goto-char marker)
                                             (insert output)
                                             (set-marker marker (+ (length output) (marker-position marker))))))))
                        :on-finished (lambda (input output success)
                                       (when on-finished
                                         (funcall on-finished input output success))
                                       ;; Still need to put overlays in the *shell* buffer
                                       (with-current-buffer shell-buffer
                                         (chatgpt-shell--put-source-block-overlays)))))))
           (goto-char (point-max)) ; Ensure we are at the end in the shell buffer
           (send)))))

   ;; Default 'shell' response style
   (t
    (let ((shell-buffer (chatgpt-shell--get-target-buffer nil))) ; Use selection logic
      (unless (buffer-live-p shell-buffer)
        (error "Could not get a valid chatgpt-shell buffer"))
      ;; Ensure the target shell is running
       (with-current-buffer shell-buffer
         (unless (comint-check-proc shell-buffer)
            (chatgpt-shell-start :no-focus nil))) ; Start if needed, focus it

      ;; Send command within the selected shell window/buffer
      (with-selected-window (get-buffer-window shell-buffer t) ; Get or select window
        (cl-flet ((send ()
                    (when shell-maker--busy
                      (shell-maker-interrupt nil))
                    (if review
                      ...
                      (shell-maker-submit
                       :input input
                       ;; No :on-output needed here, shell-maker handles shell insertion
                       :on-finished (lambda (input output success)
                                      (when on-finished
                                        (funcall on-finished input output success))
                                      (chatgpt-shell--put-source-block-overlays))))))
